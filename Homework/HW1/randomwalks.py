...
def run_saw():
    Ns = np.unique(np.logspace(2.5, 3.5, 15).astype(int))

    N, R2, Rrms, accs = estimate_scaling_pivot(Ns, n_steps=2000000, burn_in=500000, thin=1000, seed=66357)
...
def run_osaw():
    Ns = np.unique(np.logspace(2, 3.5, 15).astype(int))
    p_opens = (0.8, 0.5, 0.2)

    results = run_obstacle_growth_sweep(Ns, p_opens=p_opens, n_samples=10000, seed=66357, obstacle_seed=123, max_restarts=10000)
...
def run_confined2D():
    ...
    for D in Ds:
        ...
        (m_w, b_w), (m_s, b_s), info_r = fit_two_regimes_by_crossover(N, Rrms, D, nu_2d=0.75, margin=1.5, min_pts=3)

        (m2_w, b2_w), (m2_s, b2_s), info_2 = fit_two_regimes_by_crossover(N, R2, D, nu_2d=0.75, margin=1.5, min_pts=3)
        ...
        out_file = Path(__file__).resolve().parent / f"saw_channel_diameter_{D}_3.csv"
        data = np.column_stack((N, R2))
        np.savetxt(out_file, data, delimiter=",", comments="")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser()
    parser.add_argument("--mode", required=True, choices=["rw", "saw", "osaw", "confined2D"], help="Which simulation to run")
