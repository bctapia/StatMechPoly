...
def make_initial_chain(N, b0=1.0, mode="coil", seed=None, min_dist=0.9):
    ...
    if mode != "coil":
        raise ValueError("Only init='coil' is currently implemented.")
...
def lj_energy(r, epsilon=1.0, sigma=1.0, rcut=3.0):
    """
    Shifted 12-6 LJ energy.
    Excludes bonded neighbors with |i-j| <= 1.
    """
    N = len(r)
    e = 0.0

    sr_c = sigma / rcut
    u_shift = 4.0 * epsilon * (sr_c**12 - sr_c**6)
    ...
    for i in range(N - 2):
        ...
        if np.any(mask):
            sr = sigma / d[mask]
            e += np.sum(4.0 * epsilon * (sr**12 - sr**6) - u_shift)
...
def local_energy_for_bead(r, i, k=100.0, b0=1.0, epsilon=1.0, sigma=1.0, rcut=3.0):
    """
    Energy terms involving bead i only.
    Must match total_energy exactly for valid local MC moves.
    """
    N = len(r)
    e = 0.0
...
#run_mc_grid(
#    chain_lengths=(50, 100, 150, 200),
#    temperatures=np.arange(0.4,3.01,0.01),
#    n_steps=2_000_000,
#    sample_every=1000,
#    burn_in=200_000,
#    outdir="mc_output",
...
if __name__ == "__main__":
    ...
    if args.task_id is None:
        raise ValueError("You must pass --task-id for SLURM array execution")

    chain_lengths = [50, 100, 150, 200]
    temperatures = np.round(np.linspace(1.0, 5.0, 40), 2)

    num_T = len(temperatures)
    num_tasks = len(chain_lengths) * num_T

    if args.task_id < 0 or args.task_id >= num_tasks:
        raise ValueError(f"task_id must be between 0 and {num_tasks - 1}")

    N = chain_lengths[args.task_id // num_T]
    T = temperatures[args.task_id % num_T]

    print(f"Running task {args.task_id}: N={N}, T={T}")
